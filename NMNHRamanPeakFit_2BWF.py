...
# Loops to fit all spectra in folder
    ...
    if fnmatch.fnmatch(file, '*.txt'):
        ...
        for line in txtFile:
            ...
            if '#Laser' in lineList[i]: 
                Ext_Lambda = float(lineList[i].split()[1][:3])
                #print Ext_Lambda
            if '#Acq.' in lineList[i]:
                AcqTime = int(lineList[i].split()[3])
                #print AcqTime
            if '#Accumulations' in lineList[i]:
                NumAcq = int(lineList[i].split()[1])
                #print NumAcq
            i = i+1
        CollDet = str(NumAcq) + 'sc ' + str(AcqTime) + 's'
        #print CollDet
        ...
        #plt.show()
        fname = str(RefMat) + '_base_2LS.jpg'
        plt.savefig(fname)
        plt.close()
        ...
        plt.savefig(RefMat + 'fit_2BWFLS.jpg')
        #plt.show()
        plt.close()
        '''        
        ...
        plt.savefig(RefMat + '_Ratio_2BWFLS.jpg')
        
        #plt.show()
        plt.close()
        '''   '''
        
        f = open(RefMat+'_2BWFLS_fitfile.txt',"w")
