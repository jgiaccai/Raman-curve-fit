...
def EnterData():
    ...
    Gfit = FitGOn*Gaussian(FitParam[0],FitParam[7],FitParam[14])
    Dfit = FitDOn*Gaussian(FitParam[1],FitParam[8],FitParam[15])
    D2fit = FitD2On*Gaussian(FitParam[2],FitParam[9],FitParam[16])
    D3fit = FitD3On*Gaussian(FitParam[3],FitParam[10],FitParam[17])
    D4fit = FitD4On*Gaussian(FitParam[4],FitParam[11],FitParam[18])
    TPfit = FitTPOn*Gaussian(FitParam[5],FitParam[12],FitParam[19])
...
def FitFunc(x_fit, *EvalSimp):   
    ...
    Gfit = FitGOn*Gaussian(EvalSimp[0],EvalSimp[7],EvalSimp[14])
    Dfit = FitDOn*Gaussian(EvalSimp[1],EvalSimp[8],EvalSimp[15])
    D2fit = FitD2On*Gaussian(EvalSimp[2],EvalSimp[9],EvalSimp[16])
    D3fit = FitD3On*Gaussian(EvalSimp[3],EvalSimp[10],EvalSimp[17])
    D4fit = FitD4On*Gaussian(EvalSimp[4],EvalSimp[11],EvalSimp[18])
    TPfit = FitTPOn*Gaussian(EvalSimp[5],EvalSimp[12],EvalSimp[19])
...
def FitFuncWithUnc(x_fit, *EvalSimp):   
    ...
    Gfit = FitGOn*GaussianWithUnc(EvalSimp[0],EvalSimp[7],EvalSimp[14])
    Dfit = FitDOn*GaussianWithUnc(EvalSimp[1],EvalSimp[8],EvalSimp[15])
    D2fit = FitD2On*GaussianWithUnc(EvalSimp[2],EvalSimp[9],EvalSimp[16])
    D3fit = FitD3On*GaussianWithUnc(EvalSimp[3],EvalSimp[10],EvalSimp[17])
    D4fit = FitD4On*GaussianWithUnc(EvalSimp[4],EvalSimp[11],EvalSimp[18])
    TPfit = FitTPOn*GaussianWithUnc(EvalSimp[5],EvalSimp[12],EvalSimp[19])
...
for file in os.listdir('.'):
    if fnmatch.fnmatch(file, '*.txt'):
        ...
        for n in range(1,len(xy_data.columns)):
            ...
            Gfit_nom = FitGOn*Gaussian(fit_results[0],fit_results[7],fit_results[14])
            Dfit_nom = FitDOn*Gaussian(fit_results[1],fit_results[8],fit_results[15])
            D2fit_nom = FitD2On*Gaussian(fit_results[2],fit_results[9],fit_results[16])
            D3fit_nom = FitD3On*Gaussian(fit_results[3],fit_results[10],fit_results[17])
            D4fit_nom = FitD4On*Gaussian(fit_results[4],fit_results[11],fit_results[18])
            TPfit_nom = FitTPOn*Gaussian(fit_results[5],fit_results[12],fit_results[19])            
            ...
            TPfit = FitTPOn*GaussianWithUnc(TPloc,TPwid,TP_ints)
